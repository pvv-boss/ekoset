...
export default class ConfirmResetPassworCallback extends Vue {
  ...
  private async mounted() {
    ...
    if (!this.code) {
      ...
      const result: ResetPasswordResult = await ServiceRegistry.instance
        .getService(AuthService)
        .confirmResetPasswordByCode(this.code);

      if (result?.status === ResetPasswordStatus.OK) {
        routeName = "user-change-password";
      } else {
        BrcDialogPlugin.showNotify(BrcDialogType.Info, "Сброс пароля", result.message, 1500, {
