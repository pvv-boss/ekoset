...
const requiresAuthorize = async (ctx: Context) => {
    ...
    const allowAll = ctx.route.matched.some((record) => record.meta?.allowAll);
    ...
    if (isAuthRoute) {
        ...
        let isAllowed = isUserAuth && ((isRequiresAdmin && isUserAdmin) || (!isRequiresAdmin && !isUserAdmin) || allowAll);
