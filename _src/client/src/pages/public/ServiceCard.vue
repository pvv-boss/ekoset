<template>
  <main>
    ...
    <a class="brc-service-buscet__icon_desktop" @click="addServiceToBuscet">
      ...
      <img src="~/assets/images/addBusket.svg" :alt="basketImageInfo.alt" :title="basketImageInfo.alt" />
    </a>
    ...
    <DynamicComponentsContainer :dynamic-component-info="dynamicComponentInfo"></DynamicComponentsContainer>
  </main>
</template>
...
export default class ServiceCard extends Vue {
  ...
  private updateBasketImage() {
    ...
    const info =
      ind === -1
        ? { src: require("~/assets/images/addBusketWhite.svg"), alt: "Добавить в корзину" }
        : { src: require("~/assets/images/checkBasketWhite.svg"), alt: "В корзине" };
  ...
  private addServiceToBuscet() {
    const ind = findAddedServiceIndex(
      this.buscetStore.addedServiceList,
      BusinessServiceLocalStorageItem.createFromBusinessService(this.businessService)
    );
    if (ind === -1) {
  ...
  private async asyncData(context: Context) {
    ...
    const dynamicComponentInfo = ServiceRegistry.instance
      ...
      .getServiceDynamicComponentsInfo(siteSection, context.params.service);
