...
export default class DynamicComponentsService extends BaseService {
    public async getSiteSectionDynamicComponentsInfo(siteSectionUrl: string, adminMode = false): Promise<DynamicComponentInfo[]> {
        return this.getDynamicComponentsInfo(
            this.adminGetDynamicComponentsInfo(siteSectionUrl, null, null),
            siteSectionUrl,
            null,
            adminMode,
            null,
    ...
    public async getServiceDynamicComponentsInfo(
        siteSectionUrl: string,
        serviceUrl: string,
        adminMode = false
    ): Promise<DynamicComponentInfo[]> {
        return this.getDynamicComponentsInfo(
            this.adminGetDynamicComponentsInfo(null, serviceUrl, null),
            siteSectionUrl,
            serviceUrl,
            adminMode,
            null,
    ...
    public async getIndOfferDynamicComponentsInfo(
        siteSectionUrl: string,
        indOfferUrl: string,
        offerForClentType: string | null,
        clActivityId: number | null,
        adminMode = false
    ): Promise<DynamicComponentInfo[]> {
        return this.getDynamicComponentsInfo(
            this.adminGetDynamicComponentsInfo(null, null, indOfferUrl),
            siteSectionUrl,
            null,
            adminMode,
            indOfferUrl,
    ...
    public async getSitePageDynamicComponentsWithSiteSection(
        siteSectionUrl: string,
        sitePageId: number,
        adminMode = false
    ): Promise<DynamicComponentInfo[]> {
    ...
    private async adminGetDynamicComponentsInfo(
        siteSectionUrl: string | null,
        serviceUrl: string | null,
        indOfferUrl: string | null
    ): Promise<DynamicComponentInfo[]> {
    ...
    private createRequestQueryString(siteSectionUrl: string | null, serviceUrl: string | null, indOfferUrl: string | null) {
        ...
        const queryParam =
            id === 0 ? "default" : siteSectionUrl ? "sitesection" : serviceUrl ? "service" : indOfferUrl ? "offer" : "default";
        return `admin/panel/cms/blocks/info/${queryParam}/${id}`;
    }

    private async getDynamicComponentsInfo(
        httpResultPr: Promise<DynamicComponentInfo[]>,
        ...
        if (serviceSlug) {
            ...
            serviceIdForRelations =
                !!businessService.businessServiceParentId && businessService.businessServiceParentId > 0
                    ? businessService.businessServiceParentId
        ...
        if (serviceIdForRelations > 0) {
            brandItems = this.context.$serviceRegistry
                .getService(PublicEkosetService)
                .getBrandsByBusinessServiceSlug("slug-" + serviceIdForRelations);
        }

        if (!brandItems && !!siteSectionSlug) {
            brandItems = this.context.$serviceRegistry
                .getService(PublicEkosetService)
                .getBrandsBySiteSectionSlug(siteSectionSlug);
        }
        ...
        if (serviceIdForRelations > 0) {
            articleItems = this.context.$serviceRegistry
                .getService(ArticleService)
                .getArticleListByBusinessServiceSlug(siteSectionSlug, "slug-" + serviceIdForRelations);
        }

        if (!articleItems && !!siteSectionSlug) {
            articleItems = this.context.$serviceRegistry
                .getService(ArticleService)
                .getArticleListBySiteSectionSlug(siteSectionSlug);
        }
        ...
        if (serviceSlug) {
            const businessService = await this.context.$serviceRegistry.getService(BusinessServiceService).getBySlug(serviceSlug);
            const childServiceList = await this.context.$serviceRegistry
                .getService(BusinessServiceService)
                .getChildServicesByParentId(businessService.businessServiceId);
            serviceList =
                !!childServiceList && childServiceList.length && childServiceList.length > 0
                    ? [...childServiceList]
                    : [businessService];
            serviceHasParent = !!businessService.businessServiceParentId && businessService.businessServiceParentId > 0;

            priceList = serviceHasParent
                ? this.context.$serviceRegistry
                      .getService(BusinessServiceService)
                      .getPriceListForChildService(businessService.businessServiceId)
                : this.context.$serviceRegistry
                      .getService(BusinessServiceService)
                      .getPriceListForService(businessService.businessServiceId);

            relatedServiceList = this.context.$serviceRegistry
                .getService(BusinessServiceService)
                .adminGetRelated(businessService.businessServiceId);
        }
        ...
        if (!!indOfferUrl && !!siteSectionSlug) {
            if (offerForClentType) {
                serviceList =
                    offerForClentType === "business"
                        ? this.context.$serviceRegistry
                              .getService(BusinessServiceService)
                              .getForBusinessBySiteSectionSlug(siteSectionSlug)
                        : this.context.$serviceRegistry
                              .getService(BusinessServiceService)
                              .getForPrivatePersonBySiteSectionSlug(siteSectionSlug);

                priceList =
                    offerForClentType === "business"
                        ? this.context.$serviceRegistry
                              .getService(BusinessServiceService)
                              .getPriceListForBusinessBySiteSectionId(siteSectionSlug)
                        : this.context.$serviceRegistry
                              .getService(BusinessServiceService)
                              .getPriceListForPersonBySiteSectionId(siteSectionSlug);
            } else {
                serviceList = this.context.$serviceRegistry
                    .getService(BusinessServiceService)
                    .getByActivityAndBySiteSectionSlug(siteSectionSlug, clActivityId);
                priceList = this.context.$serviceRegistry
                    .getService(BusinessServiceService)
                    .getPriceListForActivity(siteSectionSlug, clActivityId);
            }
        }
        ...
        if (!serviceList && !!siteSectionSlug) {
            serviceList = this.context.$serviceRegistry
                .getService(BusinessServiceService)
                .getBySiteSectionSlug(siteSectionSlug, true);
            priceList = this.context.$serviceRegistry
                .getService(BusinessServiceService)
                .getPriceListForSiteSection(siteSectionSlug);
        }
        ...
        if (!!siteSectionSlug && !adminMode) {
            busineesTypeOfferList = this.context.$serviceRegistry
                .getService(IndividualOfferService)
                .getForActivityBySiteSectionIdSlug(siteSectionSlug);
        }
        ...
        if (newsCompoenentInfo) {
            ...
            newsCompoenentInfo.props.articleList =
                newsCompoenentInfo.props.articleList && newsCompoenentInfo.props.articleList.slice
                    ? newsCompoenentInfo.props.articleList.slice(0, 3)
                    : newsCompoenentInfo.props.articleList;
            newsCompoenentInfo.props.mode = "columns";

            if (!adminMode && newsCompoenentInfo.visible === 1) {
                newsCompoenentInfo.visible = 0;
                newsCompoenentInfo.visible =
                    !!newsCompoenentInfo.props.articleList && newsCompoenentInfo.props.articleList.length > 0 ? 1 : 0;
            }
        }
        ...
        if (lettersCompoenentInfo) {
            ...
            if (!adminMode && lettersCompoenentInfo.visible === 1) {
                ...
                lettersCompoenentInfo.visible =
                    !!lettersCompoenentInfo.props.brandsList && lettersCompoenentInfo.props.brandsList.length > 0 ? 1 : 0;
            }
        }
        ...
        if (!!recommendCompoenentInfo && recommendCompoenentInfo.visible === 1) {
            ...
            if (!adminMode) {
                ...
                recommendCompoenentInfo.visible =
                    !!recommendCompoenentInfo.props.brandList && recommendCompoenentInfo.props.brandList.length > 0 ? 1 : 0;
            }
        }
        ...
        if (!!busineesTypeOfferListInfo && busineesTypeOfferListInfo.visible === 1) {
            ...
            if (!adminMode) {
                ...
                busineesTypeOfferListInfo.visible =
                    !!busineesTypeOfferListInfo.props.offerList && busineesTypeOfferListInfo.props.offerList.length > 0 ? 1 : 0;
            }
        ...
        if (!!serviceInfo && serviceInfo.visible === 1) {
            ...
            if (!adminMode) {
                ...
                if (serviceSlug) {
                    serviceInfo.visible =
                        !serviceHasParent && !!serviceInfo.props.serviceList && serviceInfo.props.serviceList.length > 1 ? 1 : 0;
        ...
        if (!!servicePriceInfo && servicePriceInfo.visible === 1) {
            ...
            if (!adminMode) {
                ...
                servicePriceInfo.visible =
                    !!servicePriceInfo.props.servicePriceList && servicePriceInfo.props.servicePriceList.length > 0 ? 1 : 0;
            }
        }
        ...
        if (!!relatedServiceInfo && relatedServiceInfo.visible === 1) {
            ...
            if (!adminMode) {
                ...
                relatedServiceInfo.visible =
                    !!relatedServiceInfo.props.serviceList && relatedServiceInfo.props.serviceList.length > 0 ? 1 : 0;
            }
        }
