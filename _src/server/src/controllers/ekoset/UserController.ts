...
export default class UserController extends BaseController {
    ...
    public async getApplicationUserById(@Res() response: Response, @Param("id") id: number) {
    ...
    @UseBefore(authorized())
    @Post("/personal/user")
    public async save(@Body() client: EkosetClient, @Res() response: Response) {
        ...
        return this.createSuccessResponseWithMessage(
            ...
            response,
            200,
            ClientNotifyMessage.createAlert("ЭКОСЕТЬ", "Данные Аккаунта сохранены")
        );
    }
}
