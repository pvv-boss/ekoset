...
export default class MediaService extends BaseService {
    ...
    public async saveSiteSectionImage(siteSectionId: number, file: Express.Multer.File, isBig: boolean) {
        const updateStmt = `UPDATE site_section SET site_section_img_${
            isBig ? "big" : "small"
        } = $1 where site_section_id = ${siteSectionId}`;
    ...
    public async saveServiceImage(serviceId: number, file: Express.Multer.File, isBig: boolean) {
        const updateStmt = `UPDATE business_service SET business_service_img_${
            isBig ? "big" : "small"
        } = $1 where business_service_id = ${serviceId}`;
    ...
    public async saveOfferImage(offerId: number, file: Express.Multer.File, isBig: boolean) {
        const updateStmt = `UPDATE individual_offer SET ind_offer_img_${
            isBig ? "big" : "small"
        } = $1 where ind_offer_id = ${offerId}`;
    ...
    public async saveNewsImage(newsId: number, file: Express.Multer.File, isBig: boolean) {
        const updateStmt = `UPDATE article SET article_${
            isBig ? "header_img_src" : "preview_img_src"
        } = $1 where article_id = ${newsId}`;
    ...
    public getImageFullPathAndFileName(dir: string, subDir: string, fileprefix: string, fileExt: string, useCuid = true) {
        ...
        const fileName = useCuid ? `${fileprefix}_${cuid()}.${fileExt}` : `${fileprefix}.${fileExt}`;
    ...
    public async saveFileFromRequestBody(
    ...
    private async updateSmallOrBigImageFor(
        file: Express.Multer.File,
        imageStaticSubFolderName: string,
        filePrefix: string,
        updateStmt: string
    ) {
