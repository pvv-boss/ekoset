...
export class UserRequestService extends BaseService {
            ...
            if (!!files && files.length > 0) {
                for (const iterFile of files) {
                    const pathAndFileName = await ServiceRegistry.instance
                        ...
                        .saveFileFromRequestBody(iterFile, "app", "requests", `req${request.userRequestId}`);
    ...
    public async continionWork(clinet: EkosetClient, work: Work) {
        ...
        const title =
            work.sheldServicePlanInd === 1
                ? `Заявка на изменение даты работ с  ${work.clientName}`
                : `Заявка на повтор работы с  ${work.clientName}`;
    ...
    public sendActivateUserMail(clinet: EkosetClient | EkosetManager, password: string) {
        const format = (template: string) => {
    ...
    private formatAllPossible(
        template: string,
        clinet: EkosetClient | EkosetManager,
        contract: Contract,
        work: Work,
        sanDoc: SanDoc
    ) {
        ...
        if (!!contract) {
            result = result
                .replace(
                    "{{contractForm}}",
                    contract.contractForm + " от " + ruDateFormat.format(new Date(contract.contractDateStart))?.split(",")[0]
                )
        ...
        if (!contract && !!work) {
            result = result
                .replace(
                    "{{contractForm}}",
                    work.contractForm + " от " + ruDateFormat.format(new Date(work.contractDateStart))?.split(",")[0]
                )
        ...
        if (!contract && !!sanDoc) {
            result = result
                .replace(
                    "{{contractForm}}",
                    sanDoc.contractForm + " от " + ruDateFormat.format(new Date(sanDoc.contractDateStart))?.split(",")[0]
                )
        ...
        result =
            !!sanDoc && !!sanDoc.documentDateStart
                ? result.replace("{{documentDateStart}}", ruDateFormat.format(new Date(sanDoc.documentDateStart))?.split(",")[0])
                : result;
        result =
            !!sanDoc && !!sanDoc.documentDateEnd
                ? result.replace("{{documentDateEnd}}", ruDateFormat.format(new Date(sanDoc.documentDateEnd))?.split(",")[0])
                : result;
        result =
            !!work && !!work.sheldServiceDate
                ? result.replace("{{sheldServiceDate}}", ruDateFormat.format(new Date(work.sheldServiceDate))?.split(",")[0])
                : result;
        result =
            !!work && !!work.sheldServiceNewDate
                ? result.replace(
                      "{{sheldServiceNewDate}}",
                      ruDateFormat.format(new Date(work.sheldServiceNewDate))?.split(",")[0]
                  )
                : result;
    ...
    private async sendEmailsByRequest(request: UserRequest, mode: number, attachments: any[], clinet?: EkosetClient) {
        const format = (template: string) => {
            ...
            let startMessage =
                mode === 0
                    ? "Мы получили Ваш заказ и приступили к его обработке:"
                    : mode === 1
                    ? "Мы получили Ваш вопрос и приступили к его обработке:"
                    : "Мы получили Ваше приглашение на тендер и приступили к его обработке:";
